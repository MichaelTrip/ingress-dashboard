...
def get_ingress_resources():
    ...
    try:
        ...
        for ing in ingresses.items:
            ...
            hostname = ing.spec.rules[0].host if ing.spec.rules and ing.spec.rules[0].host else 'N/A'
            ingress_class = ing.spec.ingress_class_name or getattr(ing.spec, 'backend', None).resource.kind if getattr(ing.spec, 'backend', None) else 'Default'
